...
import {
    ...
    deleteAdvertisement,
    updateAdvertisement,
} from "@/_actions/advertisement-actions";
...
function EditForm({ ad, onDone }: { ad: Advertisement; onDone: () => void }) {
    ...
    const set =
        (key: keyof EditFields) => (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) =>
            setFields((f) => ({ ...f, [key]: e.target.value }));

    const handleSave = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!fields.label || !fields.imageUrl || !fields.linkUrl) {
            toast.error("Label, image URL and link URL are required.", { position: "top-center" });
            return;
        }
        setSaving(true);
        try {
            ...
            if (res.success) {
                toast.success("Ad updated.", { position: "top-center" });
                router.refresh();
                onDone();
            } else {
...
export default function AdList({ ads }: { ads: Advertisement[] }) {
    ...
    const toggle = async (ad: Advertisement) => {
        ...
        try {
            ...
            if (res.success) {
                ...
                toast.error(res.error ?? "Failed to update.", { position: "top-center" });
            }
        } finally {
            setBusy(null);
        }
    ...
    const remove = async (ad: Advertisement) => {
        ...
        try {
            ...
            if (res.success) {
                toast.success("Ad deleted.", { position: "top-center" });
                router.refresh();
            } else {
                toast.error(res.error ?? "Failed to delete.", { position: "top-center" });
            }
        } finally {
            setBusy(null);
        }
    ...
    return (
        <div className="space-y-3">
            {ads.map((ad) => (
                <div key={ad.id} className="rounded-xl border p-4 bg-popover shadow-sm">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            {(ad.startsAt || ad.endsAt) && (
                                <p className="text-xs text-muted-foreground mt-0.5">
                                    {ad.startsAt
                                        ? new Date(ad.startsAt).toLocaleDateString("sv-SE")
                                        : "—"}
                                    {" → "}
                                    {ad.endsAt
                                        ? new Date(ad.endsAt).toLocaleDateString("sv-SE")
                                        : "ongoing"}
                                </p>
                            )}
                            <a
                    ...
                    {editing === ad.id && <EditForm ad={ad} onDone={() => setEditing(null)} />}
                </div>
            ))}
        </div>
    );
