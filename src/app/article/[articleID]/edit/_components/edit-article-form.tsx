    ...
    const form = useForm({
        ...
        onSubmit: async ({ value }) => {
            ...
            if (result.success === false && result.error) {
                ...
                toast.success("Article updated successfully", { position: "bottom-right" });
                            ...
                            {(field) => {
                                ...
                                return (
                                    <Field data-invalid={isInvalid}>
                                        ...
                                        <Textarea
                                            className="border-r border-b min-h-48"
                                            id={field.name}
                                            name={field.name}
                                            value={field.state.value}
                                            onBlur={field.handleBlur}
                        ...
                        <div className="flex gap-4">
                            <form.Field name="category" mode="array">
                                {(field) => {
                                    const isInvalid = field.state.meta.isTouched && !field.state.meta.isValid;
                                    const handleAdd = () => {
                                        const names = categoryInput.split(",").map((n) => n.trim()).filter(Boolean);
                                        names.forEach((name) => field.pushValue(name));
                                        setCategoryInput("");
                                    };
                                    return (
                                        <Field data-invalid={isInvalid} className="border p-2 flex-1">
                                            <FieldLabel>Category</FieldLabel>
                                            <div className="px-4 py-2">
                                                {field.state.value.map((name, index) => (
                                                    <span className="px-2 py-1 rounded mr-1 mt-1 text-xs" key={index}>
                                                        {`${name} `}
                                                        <button type="button" onClick={() => field.removeValue(index)} className="hover:opacity-100 opacity-50">✕</button>
                                                    </span>
                                                ))}
                                            </div>
                                            <div className="relative flex items-center">
                                                ...
                                                />
                                                <Button size="xs" type="button" onClick={handleAdd} className="absolute right-1 my-auto">Add</Button>
                                            </div>
                                            {isInvalid && <FieldError errors={field.state.meta.errors} />}
                                        </Field>
                                    );
                        ...
                        <div className="flex gap-6">
                            <form.Field name="author" mode="array">
                                {(field) => {
                                    const isInvalid = field.state.meta.isTouched && !field.state.meta.isValid;
                                    const handleAdd = () => {
                                        const names = authorInput.split(",").map((n) => n.trim()).filter(Boolean);
                                        names.forEach((name) => field.pushValue(name));
                                        setAuthorInput("");
                                    };
                                    return (
                                        <Field data-invalid={isInvalid} className="flex-1 border p-2">
