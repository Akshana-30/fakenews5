...
export default function WeatherWidget() {
  ...
  const updated = new Date(data.updatedAt).toLocaleTimeString("sv-SE", {
    hour: "2-digit",
    minute: "2-digit",
  });
  ...
  return (
    <div className="bg-muted border border-border p-3.5">
      ...
      <div className="flex items-center justify-between gap-2">
        <p className="font-serif text-[40px] font-bold leading-none text-foreground">
          {data.temp}
        </p>
        <WeatherIcon code={data.code} />
      </div>
      <p className="font-sans text-[13px] font-semibold text-foreground mt-1">
        {data.desc}
      </p>
      <p className="font-sans text-[11px] text-muted-foreground">
